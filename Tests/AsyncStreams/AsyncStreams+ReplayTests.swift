...
final class AsyncStreams_ReplayTests: XCTestCase {
    func testInit_sets_storage() async {
        ...
        let buffer = await sut.storage.elements
        let bufferSize = await sut.storage.size
        XCTAssertEqual(bufferSize, 10)
        XCTAssertTrue(buffer.isEmpty)
    }

    func testSend_pushes_element_in_storage() async {
    ...
    func testSendFinished_ends_the_streams_and_clear_the_storage() {
        ...
        let hasClearedExpectation = expectation(description: "All internal resources have been cleared")
        ...
        Task {
            let buffer = await sut.storage.buffer
            let continuations = await sut.continuations.continuations

            XCTAssertTrue(buffer.isEmpty)
            XCTAssertTrue(continuations.isEmpty)
            hasClearedExpectation.fulfill()
        }

        wait(for: [hasClearedExpectation], timeout: 1)
    }
