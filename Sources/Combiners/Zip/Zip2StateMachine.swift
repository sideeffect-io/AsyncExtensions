...
where Element1: Sendable, Element2: Sendable {
  ...
  mutating func newDemandFromConsumer(
    ...
    switch self.state {
      ...
      case .awaitingBaseResults(let task, _, _, let suspendedBases, let oldSuspendedDemand):
  ...
  mutating func newLoopFromBase1(suspendedBase: UnsafeContinuation<Void, Never>) -> NewLoopFromBaseOutput {
    switch self.state {
      ...
      case .awaitingDemandFromConsumer(let task, var suspendedBases):
        assert(suspendedBases.count < 2, "There cannot be more than 2 suspended base at the same time")
      ...
      case .awaitingBaseResults(let task, let result1, let result2, var suspendedBases, let suspendedDemand):
        assert(suspendedBases.count < 2, "There cannot be more than 2 suspended bases at the same time")
        if result1 != nil {
          suspendedBases.append(suspendedBase)
          self.state = .awaitingBaseResults(task: task, result1: result1, result2: result2, suspendedBases: suspendedBases, suspendedDemand: suspendedDemand)
          return .none
        } else {
          self.state = .awaitingBaseResults(task: task, result1: result1, result2: result2, suspendedBases: suspendedBases, suspendedDemand: suspendedDemand)
    ...
    switch self.state {
      ...
      case .awaitingBaseResults(let task, _, let result2, let suspendedBases, let suspendedDemand):
        if let result2 = result2 {
          self.state = .awaitingBaseResults(task: task, result1: .success(element), result2: result2, suspendedBases: suspendedBases, suspendedDemand: nil)
          return .resumeDemand(suspendedDemand: suspendedDemand, result1: .success(element), result2: result2)
        } else {
          self.state = .awaitingBaseResults(task: task, result1: .success(element), result2: nil, suspendedBases: suspendedBases, suspendedDemand: suspendedDemand)
          return .none
        }
    ...
    switch self.state {
      ...
      case .awaitingBaseResults(let task, let result1, _, let suspendedBases, let suspendedDemand):
        if let result1 = result1 {
          self.state = .awaitingBaseResults(task: task, result1: result1, result2: .success(element), suspendedBases: suspendedBases, suspendedDemand: nil)
          return .resumeDemand(suspendedDemand: suspendedDemand, result1: result1, result2: .success(element))
        } else {
          self.state = .awaitingBaseResults(task: task, result1: nil, result2: .success(element), suspendedBases: suspendedBases, suspendedDemand: suspendedDemand)
          return .none
        }
  ...
  mutating func demandIsFulfilled() -> DemandIsFulfilledOutput {
    switch self.state {
      ...
      case .awaitingBaseResults(let task, let result1, let result2, let suspendedBases, let suspendedDemand):
        ...
        assert(result1 != nil && result2 != nil, "Inconsistent state, all results are not yet available to be acknowledged")
        self.state = .awaitingDemandFromConsumer(task: task, suspendedBases: suspendedBases)
        return .none
